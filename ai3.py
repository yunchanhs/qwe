...
# 각 코인에 대한 데이터를 수집하고, RSI, MACD 등의 지표 추가 (TA-Lib 대신 pandas, numpy 사용)
def get_coin_data(coin, interval="day", count=1000):
    try:
        ...
        # RSI 계산 (14일 기준)
        df['RSI'] = 100 - (100 / (1 + (df['close'].diff().gt(0).rolling(window=14).sum() / 14) / (df['close'].diff().lt(0).rolling(window=14).sum() / 14)))
